...
class Website():
    ...
    def check_recent_board(self, attempted_word):
        ...
        for i in range(len(attempted_word)):
            ...
            if i == 4:
                game.send_keys(Keys.RETURN).perform()
        ...
        time.sleep(2)
        try: 
            print(returned_boxes[0].get_attribute("evluation"))
            assert(returned_boxes[0].get_attribute("evluation") != None)
        except:
            print('invalid word! trying another')
            game = webdriver.ActionChains(self.driver)
            for i in range(len(attempted_word)):
                game.send_keys(Keys.BACKSPACE)
            raise
