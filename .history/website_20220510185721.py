...
class Website():
    ...
    def check_recent_board(self, attempted_word):
        ...
        right_row = self.driver.execute_script(
            f"return document.querySelector('game-app').shadowRoot.querySelector('game-theme-manager').querySelector('#game').querySelector('#board').children[{self.attempts}].shadowRoot")
        self.driver.implicitly_wait(15)

        print(right_row)
        # assert(len(right_row) > 0)
